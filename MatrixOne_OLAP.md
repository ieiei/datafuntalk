...
比如说TPC-H Q19的那个过滤条件是三个很长的谓词用or来连接起来，通过观察实际上这三个or里面其实有很多共同的部分，咱们可以把共同的部分提取出来，变成右图的样子。
...
1. 第一步是确定事实表和维度表，因为一般的OLAP查询的数据通常会把表分成事实表和维度表
   无论是星型模型还是雪花模型，他都是会分事实表和维度表。那事实表维度表之间可以使用维度表的主键去做join。
...
3. 那最后一步的是在子树之间，像TPC DS的表，会有多个维度表，这维度表互相之间都不是以主键来做join。那么在子树之间，我们再使用经典的join order的算法，比如说像动态规划这些。
...
因为我用黄色的选项表示
![[MatrixOne-32.png]]
那么最后实际生成的最优的join顺序是从region开始，先跟nation join，再跟customer join，再跟orders join，再和lineitem join，最后跟supplier join。这是我们新推断出来的条件。
...
大家可以看一下上图的benchmark结果，前两列是在Snowflake上面做测试的，Snowflake是使用更我们机器配置价格差不多的配置。
...
为什么要跟snowflake比呢，是因为snowflake和我们的架构是差不多的，都是云原生，数据完全存算分离。数据是存储在远端，不会跟实际的计算节点存在一起
