...
这是涉及到很多设计理念的问题
而且Lakesoul是作为现有国产唯一的这种开源框架全是我们自研的，并不是基于其他的框架之上开发的。这里从底层设计到上层应用，包括一个生态系统的建设，整个链路都是我们自己实现的。
所以今天在这里详细分析一下
我们到底是怎么设计的
为什么这么做、
做之后有什么样的好处、
...
提起湖仓一体的技术背景就离不开云原生，云原生出来之后给我们带来很多这种方便快捷的体验，
比如随便拉一个镜像，搭几台集群，基本上通过一件事或几行命令就能把一个集群或者一个应用部署上去，这极大减少了我们的使用和维护成本。
并且云原生在底层，其实都是以对象存储为基础的
对象存储带来的最大的好处就是特别廉价，同时它的性能也不差，存储的格式多，比如结构化、非结构化、还有半结构化的数据等等。

对于数据分析来说，这样的一个廉价的存储就能存储的更多的数据，就想大力出奇迹的似得，会带来很多意想不到的收获。

首先是带来很多模式的转变，比如说现在就是说大家都后来就是说
呃对象存储之后，有一段时间特别兴起的一个名词：”数据入湖“，同时相应的出现了一个新的工种： “入湖工程师”。

对于数据，为什么首先入湖，而不是说像传统模式，数据结合使用方先做处理。
